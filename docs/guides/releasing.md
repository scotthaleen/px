...
The workflow attaches six archives plus `SHA256SUMS`. Linux and macOS archives use `.tar.gz`; Windows archives use `.zip`. Each platform has amd64 and arm64 builds, and each archive includes `px`, `px-server`, the release manifest, license, and notices. Builds have no deployment-specific rendezvous default. No private installer or local deployment artifact is uploaded.
