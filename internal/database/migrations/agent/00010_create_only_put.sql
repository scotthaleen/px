-- +goose Up
ALTER TABLE contexts ADD COLUMN put_root TEXT CHECK (put_root IS NULL OR (length(put_root) BETWEEN 1 AND 4096));
ALTER TABLE contexts ADD COLUMN allow_put INTEGER NOT NULL DEFAULT 0 CHECK (allow_put IN (0, 1));
ALTER TABLE contexts ADD COLUMN put_root_revision INTEGER NOT NULL DEFAULT 1 CHECK (put_root_revision > 0);
...
CREATE TABLE put_transfers (
    ...
    CHECK ((direction = 'send' AND source_path IS NOT NULL AND parent_path IS NULL AND parent_identity IS NULL AND stage_name IS NULL AND stage_identity IS NULL AND stage_removed = 0 AND parent_synced = 0)
...
-- Shared sender/receiver admission is enforced by SQLite, not racing process checks.
...
DROP TABLE put_transfers;
ALTER TABLE contexts DROP COLUMN put_root_revision;
ALTER TABLE contexts DROP COLUMN allow_put;
ALTER TABLE contexts DROP COLUMN put_root;
