...
ALTER TABLE put_transfers RENAME TO put_transfers_create_only;
...
CREATE TABLE put_transfers (
    ...
    CHECK ((direction = 'send' AND source_path IS NOT NULL AND parent_path IS NULL AND parent_identity IS NULL AND stage_name IS NULL AND stage_identity IS NULL AND old_identity IS NULL AND old_uid IS NULL AND old_gid IS NULL AND old_mode IS NULL AND stage_removed = 0 AND parent_synced = 0)
...
INSERT INTO put_transfers(direction,transfer_id,protocol_version,context_name,peer_device_id,local_device_id,peer_label,destination,mode,source_size,source_sha256,chunk_size,ack_window,put_root_revision,source_path,parent_path,parent_identity,stage_name,stage_identity,state,acknowledged_bytes,created,durability,stage_removed,parent_synced,created_at,updated_at,expires_at,next_retry_at,lease_token,lease_expires_at)
SELECT direction,transfer_id,2,context_name,peer_device_id,local_device_id,peer_label,destination,mode,source_size,source_sha256,chunk_size,ack_window,put_root_revision,source_path,parent_path,parent_identity,stage_name,stage_identity,state,acknowledged_bytes,created,durability,stage_removed,parent_synced,created_at,updated_at,expires_at,next_retry_at,lease_token,lease_expires_at FROM put_transfers_create_only;
DROP TABLE put_transfers_create_only;
...
-- +goose Down
CREATE TEMP TABLE put_replacement_rollback_guard(value INTEGER CHECK (value = 0));
INSERT INTO put_replacement_rollback_guard SELECT count(*) FROM put_transfers WHERE mode != 'create';
DROP TABLE put_replacement_rollback_guard;
DROP TRIGGER transfer_shared_quota_insert;
DROP TRIGGER put_shared_quota_insert;
DROP INDEX put_transfers_due_parent;
DROP INDEX put_transfers_context_updated;
ALTER TABLE put_transfers RENAME TO put_transfers_replacement;
...
CREATE TABLE put_transfers (
    ...
    peer_label TEXT NOT NULL CHECK (length(peer_label) BETWEEN 1 AND 255),
    destination TEXT NOT NULL CHECK (length(destination) BETWEEN 1 AND 4096),
    mode TEXT NOT NULL CHECK (mode = 'create'),
    source_size INTEGER NOT NULL CHECK (source_size BETWEEN 0 AND 1073741824),
    source_sha256 TEXT NOT NULL CHECK (length(source_sha256) = 64 AND source_sha256 NOT GLOB '*[^0-9a-f]*'),
    chunk_size INTEGER NOT NULL CHECK (chunk_size = 32768),
    ack_window INTEGER NOT NULL CHECK (ack_window = 8),
    ...
    PRIMARY KEY (direction, transfer_id),
    CHECK ((lease_token IS NULL) = (lease_expires_at IS NULL)),
    CHECK ((direction = 'send' AND source_path IS NOT NULL AND parent_path IS NULL AND parent_identity IS NULL AND stage_name IS NULL AND stage_identity IS NULL AND stage_removed = 0 AND parent_synced = 0) OR (direction = 'receive' AND source_path IS NULL)),
    ...
    CHECK (parent_synced = 0 OR stage_removed = 1),
    CHECK (stage_removed = 0 OR (direction = 'receive' AND stage_name IS NOT NULL AND state IN ('cleanup_not_attempted','committed'))),
    CHECK ((state = 'stage_intent' AND direction = 'receive' AND stage_name IS NOT NULL AND stage_identity IS NULL AND stage_removed = 0 AND created = 0 AND durability IS NULL)
        OR (state = 'transferring' AND created = 0 AND durability IS NULL AND (direction = 'send' OR (stage_identity IS NOT NULL AND stage_removed = 0)))
        OR (state = 'publication_intent' AND direction = 'receive' AND stage_identity IS NOT NULL AND created = 0 AND durability IS NULL)
        OR (state = 'cleanup_not_attempted' AND direction = 'receive' AND stage_name IS NOT NULL AND created = 0 AND durability IS NULL)
        OR (state = 'committed' AND created = 1 AND durability IS NOT NULL)
        OR (state = 'outcome_unknown' AND direction = 'receive' AND stage_identity IS NOT NULL AND durability IS NULL))
);
INSERT INTO put_transfers SELECT direction,transfer_id,context_name,peer_device_id,local_device_id,peer_label,destination,mode,source_size,source_sha256,chunk_size,ack_window,put_root_revision,source_path,parent_path,parent_identity,stage_name,stage_identity,state,acknowledged_bytes,created,durability,stage_removed,parent_synced,created_at,updated_at,expires_at,next_retry_at,lease_token,lease_expires_at FROM put_transfers_replacement;
DROP TABLE put_transfers_replacement;
CREATE INDEX put_transfers_context_updated ON put_transfers(context_name, updated_at DESC, direction, transfer_id);
CREATE INDEX put_transfers_due_parent ON put_transfers(parent_path, parent_identity, next_retry_at, created_at, transfer_id);
...
CREATE TRIGGER put_shared_quota_insert BEFORE INSERT ON put_transfers BEGIN
    SELECT CASE WHEN NEW.direction = 'send' AND NOT EXISTS (SELECT 1 FROM put_transfers WHERE direction=NEW.direction AND transfer_id=NEW.transfer_id) AND
        (SELECT count(*) FROM transfer_resumes WHERE direction = 'send') + (SELECT count(*) FROM put_transfers WHERE direction = 'send') >= 64
        THEN RAISE(ABORT, 'shared sender transfer capacity reached') END;
    SELECT CASE WHEN NEW.direction = 'receive' AND NOT EXISTS (SELECT 1 FROM put_transfers WHERE direction=NEW.direction AND transfer_id=NEW.transfer_id) AND
        (SELECT count(*) FROM transfer_resumes WHERE direction = 'receive' AND state IN ('transferring','committing','committed_pending_confirmation')) + (SELECT count(*) FROM put_transfers WHERE direction = 'receive' AND (state != 'committed' OR stage_identity IS NOT NULL)) >= 64
        THEN RAISE(ABORT, 'shared receiver transfer capacity reached') END;
    SELECT CASE WHEN NEW.direction = 'receive' AND NOT EXISTS (SELECT 1 FROM put_transfers WHERE direction=NEW.direction AND transfer_id=NEW.transfer_id) AND
        (SELECT coalesce(sum(source_size), 0) FROM transfer_resumes WHERE direction = 'receive' AND state IN ('transferring','committing','committed_pending_confirmation')) + (SELECT coalesce(sum(source_size), 0) FROM put_transfers WHERE direction = 'receive' AND (state != 'committed' OR stage_identity IS NOT NULL)) > 4294967296 - NEW.source_size
        THEN RAISE(ABORT, 'shared receiver byte capacity reached') END;
END;
-- +goose StatementEnd
-- +goose StatementBegin
